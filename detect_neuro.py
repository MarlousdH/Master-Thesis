...
#ADDED for PCA
    ...
    selected_neurons = []
    for layer_name, neuron_indices in safety_neurons.items():
        for i in neuron_indices:
            selected_neurons.append((layer_name, int(i)))
    selected_neurons = sorted(selected_neurons, key=lambda x: (str(x[0]), x[1]))
    results = []
    for i in range (len(labels)):
        feature_vector = []
        for (layer_name, neuron) in selected_neurons:
            feature_vector.append(float(activations[layer_name][i][neuron]))
        results.append({"label": int(labels[i]), "features": feature_vector})

    #ADDED to create json file with outputs
    import numpy as np
    import os
    out_dir = Path("outputs/neurostrike")
    out_dir.mkdir(parents=True, exist_ok=True)
    job_id = os.environ.get("SLURM_JOB_ID", "local")
    out_path = out_dir / f"safety_neurons_model{model_id}-{job_id}.json"

    # safety_neurons is already a dict:
    # { layer_name (str) : np.array of neuron indices }
    serializable_safety_neurons = {layer: np.asarray(neurons, dtype=int).tolist() for layer, neurons in safety_neurons.items()}
    payload = {"method": "neurostrike","model_id": model_id,"model_name": model_name,"safety_neurons": serializable_safety_neurons,}
    with open(out_path, "w") as f:
        json.dump(payload, f, indent=2)
    print(f"[OK] Saved safety neurons to {out_path}")

    with open(out_dir / "neurostrike_pca.json", "w") as f:
        json.dump(results, f)
