...
if "safety_neurons" in subset_payload:
    goodvibe_neurons = subset_payload["safety_neurons"]
else:
    goodvibe_neurons = subset_payload

activations = collect_pooled_activations(model=model, tokenizer=tokenizer, prompts=prompts, pool="mean")
activations = filter_activations_by_subset(activations, goodvibe_neurons)
subset_mapping = {}
for layer, idxs in goodvibe_neurons.items():
...
for layer, head in heads.items():
    ...
    print(f"\nLayer: {layer}")
    for i_local, weight in enumerate(W):
        i_global = mapping[i_local]
        rounded_weights = np.round(weight, 4)
...
for layer, head in heads.items():
    W = head.linear.weight.detach().cpu().numpy()
    subset_indices = goodvibe_neurons[layer]
    neuron_category_mapping[layer] = {}
    for c_idx, cat_name in enumerate(all_cats):
        weights = W[c_idx]
        pairs = []
        for j_local, w in enumerate(weights):
            j_global = subset_indices[j_local]
            pairs.append((int(j_global), float(w)))
        pairs = sorted(pairs, key=lambda x: abs(x[1]), reverse=True)
        neuron_category_mapping[layer][cat_name] = pairs

#pick the top 3 category weights per neuron
top_k = 5
neuron_topk_categories = {}
for layer, head in heads.items():
    W = head.linear.weight.detach().cpu().numpy()  # shape (C, H_subset)
    subset_indices = goodvibe_neurons[layer]
